...
// 数字立方和等于该数本身。例如：153是一个“水仙花数”，因为153 =
// 1的三次方 ＋5的三次方＋3的三次方。
//     程序分析：利用for循环控制100 -
...
// 打印出90 =
// 2 * 3 * 3 *
// 5。 程序分析：对n进行分解质因数，应先找到一个最小的质数k，然后按下述步骤完
// 成：
// (1)
...
void main(void) { three_hellos(); /*调用此函数*/ }
...
void main() {
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++) scanf("%f", &a[j]);
    for (i = 0; i < 3; i++) sum = sum + a;
